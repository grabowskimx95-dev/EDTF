...
with tab1:
    if pending > 0:
        ...
        if st.button(f"🔭 Scout Market for {sel_niche}"):
            # Manual trigger logic via file flag or direct DB inject
             st.info("Scout dispatched via background engine...")
             # Note: Real scouting happens in engine.py based on DB state

# --- TAB 2: PIPELINE ---
with tab2:
    # Show all items for this niche
    if niche_items:
...
# ==============================================================================
# 📦 FILE 2: THE BACKGROUND ENGINE (V45 + Niche Awareness)
# ==============================================================================
...
def run_production_real(item, keys, niches_config):
    ...
    base = f"Daily_Packet_{today}/{clean}"
    ...
    # 4. PUBLISH (Mock WP Upload for stability in script)
    # In real deployment, insert WP upload logic here using keys['wp_url']
    ...
    while True:
        try:
            ...
            with open(DB_FILE, 'r+') as f:
                ...
                # Only scout if global pending count is low (<5)
                    ...
                    for niche_name, config in data['niches'].items():
                        ...
                        for i in items:
                            ...
                            data['db'].append({
                                ...
                                "link": "", 
                                "app_url": url,
                                "niche": niche_name # Tag with Brand
                            })
...
def main():
    ...
    print(f"✅ TYCOON EDITION INSTALLED TO: {BASE_PATH}")
    print("1. Open Folder.")
